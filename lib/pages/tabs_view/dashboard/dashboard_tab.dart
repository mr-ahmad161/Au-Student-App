...
class _DashBoardTabState extends State<DashBoardTab> {
  ...
  void getStudentGroups() {
    String studentId = context.read<AuthNotifier>().user.studentID;
    Repository().getGroups(studentId: studentId, userType: "Student")
        .then((GroupsResponseModel groups) {
      context.read<AppStateNotifier>().groupsResponseModel = groups;
    });
  }
                          ...
                          subtitle: Padding(
                            ...
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                SizedBox(width: 6),
                                Text(
                                  "${DateFormat('EEE - h:mm a').format(event.registrationStart)} - ${DateFormat('h:mm a').format(event.registrationEnd)}"
                                      .toUpperCase(),
                                  style: TextStyle(fontSize: DS.setSP(13)),
                                ),
                              ],
          ...
          child: Stack(
            children: [
              GestureDetector(
                onTap: () {
                  return widget.onTap(
                      snapshot.data.announcement
                          .singleWhere((announcement) => announcement.masterId == snapshot.data.images[currentIndex].masterId)
                          .masterYear,
                ...
                children: [
                  ...snapshot.data.images.map((AnnouncementImage announcementImage) {
                    return GestureDetector(
                      onTap: () => widget.onTap(
                          snapshot.data.announcement.singleWhere((announcement) => announcement.masterId == announcementImage.masterId).masterYear,
                          announcementImage.masterId),
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(24),
